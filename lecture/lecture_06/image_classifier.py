...
def run_classifier():
    ...
    cnn_classifier = ImageClassifier_MLP(img_shape_x=28, img_shape_y=28, num_labels=10)
    cnn_classifier.build_CNN_model()
    train_labels = ImageClassifier_MLP.to_onehotvec_label(train_labels, 10)
    cnn_classifier.fit(train_imgs=train_images, train_labels=train_labels, num_epochs=epochs)

    predicted_labels = cnn_classifier.predict(test_imgs=test_images)
