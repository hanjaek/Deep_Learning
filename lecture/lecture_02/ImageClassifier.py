...
class ImageClassifier_MLP:
    ...
    def build_model(self):
        ...
        hidden_layer_1 = tf.keras.layers.Dense(units=128,
                                       activation=tf.keras.activations.relu,
                                       use_bias=True)(flatten_layer)
        
        hidden_layer_2 = tf.keras.layers.Dense(units=128,
                                       activation=tf.keras.activations.relu,
                                       use_bias=True)(hidden_layer_1)
        
        output_layer = tf.keras.layers.Dense(units=self.num_labels,
                                       activation=tf.keras.activations.softmax,
                                       use_bias=True)(hidden_layer_2)
